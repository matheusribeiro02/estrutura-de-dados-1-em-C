...
int busca_e_traz_para_o_inicio(Lista* li, int mat, struct aluno* al){
	
	if(li == NULL || mat < 1 || mat > tamanho_lista(li))
		return 0;
	...
	*li_aux = *li;
	...
	consulta_lista_mat(li, mat, &aluno);
    insere_lista_inicio(li_aux, aluno);
    remove_lista(li, mat);
    consulta_lista_mat(li_aux, mat, &aluno);
