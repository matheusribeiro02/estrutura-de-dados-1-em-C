...
int remove_n_primeiroselem(Lista* li, int n){
	
	if(tamanho_lista(li) < n || n < 1)
		return 0;
...
int divide(Lista* li,Lista* li_2,Lista* li_3, int pos){
	
	if(tamanho_lista(li) < pos || pos < 1)
        return 0;
    ...
    do{
    	if(i < pos){
    		insere_lista_final(li_2, no->dados);
		} else {
			insere_lista_final(li_3, no->dados);
		}
		i++;
		no = no->prox;
	} while(no != (*li));
...
/*int estaOrdenada(Lista* li){
    ...
    if (li != NULL)
    {
        ...
        while (no->prox != NULL)
        {
            if (no->dados.matricula > no->prox->dados.matricula)
                return 0;
            no = no->prox;
        }
    }
